...
class SpriteSheet:
    def __init__(self, files, scale):
        ...
        for i in range(len(files)):
            self.sheets.append(pygame.image.load(
                f'Assets/{self.files[i][0]}.png'))
            for j in range(len(self.sheets)):
                x_scale = int(self.sheets[j].get_width()/scale[0])
                y_scale = int(self.sheets[j].get_height()/scale[1])
                self.sheets[j] = pygame.transform.scale(
                    self.sheets[j], (x_scale, y_scale))
        ...
        for i in range(len(self.files)):
            ...
            for row in range(rows):
                for col in range(columns):
                    rect = pygame.Rect(col*im_width, row *
                                       im_height, im_width, im_height)
                    right_frames.append(self.sheets[i].subsurface(rect))
                    if len(self.files[i]) < 4:
                        left_frames.append(pygame.transform.flip(
                            self.sheets[i].subsurface(rect), True, False))
                    else:
                        tempRight_frames = []
                        ...
                        tempRight_frames.append(
                            self.sheets[i].subsurface(rect))
                        left_frames.append(pygame.transform.flip(
                            self.sheets[i].subsurface(tempRect), True, False))
